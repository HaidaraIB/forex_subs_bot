...
async def free_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type == Chat.PRIVATE:
        ...
        if user.cur_sub:
            ...
            return
        elif not context.bot_data.get("free_on", True) or not context.bot_data.get(
            "free_sub_chats", None
        ):
            await update.callback_query.answer(
        ...
        for chat_id in context.bot_data["free_sub_chats"]:
            ...
            context.job_queue.run_once(
                ...
                user_id=update.effective_user.id,
                chat_id=chat_id,
                name=f"{update.effective_user.id} {chat_id}",
                data=link.invite_link,
                job_kwargs={
                    "id": f"{update.effective_user.id} {chat_id}",
